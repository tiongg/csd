...
export default function DesmosPage() {
  ...
  const legendGroups = useMemo(() => {
    ...
    const { componentByNode } = buildComponentLookup(nodes, links);
    ...
    nodes.forEach((node) => {
      const cluster = componentByNode.get(node.id) ?? 0;
      const color =
        RELATION_CLUSTER_COLORS[cluster % RELATION_CLUSTER_COLORS.length] ??
        '#64748b';
      const existingGroup = groups.get(cluster);

      if (existingGroup) {
        existingGroup.items.push({ id: node.id });
        return;
      }
  ...
  const defaultOpenGroups = useMemo(() => {
    if (filteredLegendGroups.length === 0) {
      return [];
    }

    if (searchQuery.trim()) {
      return filteredLegendGroups.map((group) => `cluster-${group.cluster}`);
    }

    return [];
  }, [filteredLegendGroups, searchQuery]);
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-4">
        <section className={glassPanelClass}>
          ...
          <p className="mt-2 max-w-4xl text-sm text-slate-600">
            Explore term relationships visually. Search to auto-focus and zoom
        ...
        <section className={`${glassPanelClass} flex min-h-0 flex-1 flex-col`}>
          ...
          >
            <div className="relative min-h-[420px] flex-1 overflow-hidden rounded-xl border border-slate-200/80 bg-white/65 xl:min-h-0 xl:min-w-0 xl:h-[var(--desmos-panel-height)]">
              <RelationGraph
            ...
            <aside
              className="flex min-h-[420px] w-full shrink-0 flex-col overflow-hidden rounded-xl border border-slate-200/80 bg-white/82 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)] xl:min-h-0 xl:h-[var(--desmos-panel-height)] xl:max-h-[var(--desmos-panel-height)] xl:w-[var(--desmos-panel-width)]"
              style={
                {
            ...
            >
              ...
              <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
                <div className="px-1 py-2 pb-6">
                  {filteredLegendGroups.length > 0 ? (
                    <Accordion
                      ...
                      className="space-y-3"
                    >
                      {filteredLegendGroups.map((group, index) => {
                      const { r, g, b } = hexToRgb(group.color);

                      return (
                        <AccordionItem
                          key={group.cluster}
                          value={`cluster-${group.cluster}`}
                          className={cn(
                            'rounded-lg border border-slate-200/80 border-b border-slate-200/80 bg-white/75 px-3 last:border-b',
                            index === filteredLegendGroups.length - 1 && 'mb-4',
                          )}
                        >
