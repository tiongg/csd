  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-xl border-slate-200 p-5 sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle className="text-slate-900">Keyword to Course Match</DialogTitle>
                ...
                {filteredCourses.map((course) => (
                  ...
                  >
                    <div className="flex gap-3">
                      <div className="w-40 shrink-0 self-stretch overflow-hidden rounded-lg bg-slate-200">
                        {course.imageUrl ? (
                          <img
                            src={course.imageUrl}
                            alt={course.title}
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-1.5">
                          <span className="inline-flex items-center rounded-full border border-slate-300 bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-700">
                            {course.category}
                          </span>
                          {(course.tags ?? []).map((tag) => (
                            <span
                              key={`${course.id}-${tag}`}
                              className="inline-flex items-center rounded-full border border-slate-300 bg-slate-50 px-2 py-0.5 text-[11px] font-medium text-slate-600"
                            >
                              #{tag}
                            </span>
                          ))}
