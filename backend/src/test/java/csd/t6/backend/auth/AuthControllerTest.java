...
class AuthControllerTest {
    ...
    void setUp() {
        mockMvc = MockMvcBuilders
            ...
            .apply(springSecurity())   // <-- this is the key fix
            .build();

        testAccount = accountRepository.insert(
            "authtest@example.com",
            "authuser",
            "$2a$12$dummyhashedpassword123456"
        );
    }
    ...
    void shouldLoginSuccessfully() throws Exception {
        ...
        String body = objectMapper.writeValueAsString(
            new HashMap<>() {{ put("usernameOrEmail", "authuser"); put("password", "password"); }}
        );
...
void shouldReturn400ForInvalidCredentials() throws Exception {
    ...
    String body = objectMapper.writeValueAsString(
        new HashMap<>() {{ put("usernameOrEmail", "wrong"); put("password", "wrong"); }}
    );
    ...
    void shouldExchangeCodeForTokens() throws Exception {
        ...
        String body = objectMapper.writeValueAsString(
            new HashMap<>() {{ put("code", "valid_code"); }}
        );
    ...
    void shouldReturn400ForInvalidCode() throws Exception {
        when(oauthCodeService.consumeCode("bad_code"))
            .thenThrow(new BadRequestException("Invalid or expired code"));

        String body = objectMapper.writeValueAsString(
            new HashMap<>() {{ put("code", "bad_code"); }}
        );
