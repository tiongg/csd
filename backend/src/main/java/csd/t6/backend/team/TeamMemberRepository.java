...
public class TeamMemberRepository {
  ...
  public void updateRole(UUID teamId, UUID accountId, TeamRole newRole) {
    dsl.update(TEAM_MEMBER)
        .set(TEAM_MEMBER.TEAM_ROLE, newRole)
        .where(TEAM_MEMBER.TEAM_ID.eq(teamId))
        .and(TEAM_MEMBER.ACCOUNT_ID.eq(accountId))
        .execute();
  }
